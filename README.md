...
GeoHelper 是一个平台型 agent 系统。当前主线由静态前端、网关运维面、带默认 inline worker loop 的 `/api/v3` control plane，以及一组 `agent-*` 平台包组成。
...
- `apps/worker`: 可选的 standalone worker 运行时壳层
...
- Control plane 负责 `/api/v3/health`、`/api/v3/ready`、`/api/v3/threads`、`/api/v3/threads/:threadId/runs`、`/api/v3/runs/:runId/stream`、`/api/v3/checkpoints/:checkpointId/resolve`
