...
export const WorkspaceChatMessages = ({
  ...
  const renderMessageBody = (message: ChatMessage) => {
    if (message.role !== "assistant" || message.result?.status !== "success") {
      return message.content ? <div>{message.content}</div> : null;
    }

    const pendingCount =
      message.result.uncertaintyItems?.filter(
        (item) => item.reviewStatus === "pending"
      ).length ?? 0;
    const receipt =
      pendingCount > 0
        ? `结构化结果已同步到结果面板，请继续确认 ${pendingCount} 项待确认内容。`
        : "结构化结果已同步到结果面板，请在结果面板继续补图。";

    return (
      <div
        className="chat-structured-result-receipt"
        data-testid="chat-structured-result-receipt"
      >
        {receipt}
      </div>
    );
